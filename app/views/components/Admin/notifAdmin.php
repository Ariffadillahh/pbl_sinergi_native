    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (notifToggleBtn && notifDropdown) {
            ...
            document.addEventListener('click', (event) => {
                ...
                const isClickOnButton = notifToggleBtn.contains(event.target);

                if (!notifDropdown.classList.contains('hidden') && !isClickInsideDropdown && !isClickOnButton) {
            ...
            tabUnread.addEventListener('click', () => {
                ...
                tabUnread.classList.add('text-blue-600', 'border-blue-600', 'bg-white');
                tabUnread.classList.remove('text-gray-600', 'hover:text-gray-800', 'hover:bg-gray-100');

                readContainer.classList.add('hidden');
                tabRead.classList.add('text-gray-600', 'hover:text-gray-800', 'hover:bg-gray-100');
                tabRead.classList.remove('text-blue-600', 'border-blue-600', 'bg-white');
            });

            tabRead.addEventListener('click', () => {
                unreadContainer.classList.add('hidden');
                tabUnread.classList.remove('text-blue-600', 'border-blue-600', 'bg-white');
                tabUnread.classList.add('text-gray-600', 'hover:text-gray-800', 'hover:bg-gray-100');

                readContainer.classList.remove('hidden');
                tabRead.classList.remove('text-gray-600', 'hover:text-gray-800', 'hover:bg-gray-100');
                tabRead.classList.add('text-blue-600', 'border-blue-600', 'bg-white');
            });
        }
        ...
        function createNotificationHTML(notif) {
            ...
            const messageMap = {
                ...
                REPLY_COMMENT: `<strong>${DATA.sender_name || 'Someone'}</strong> replied to your comment`,
                MENTION: `<strong>${DATA.sender_name || 'Someone'}</strong> someone tagged you on a post.`,
                KICKED: `<strong>${DATA.sender_name || 'Someone'}</strong> kicked you from their group.`,
                DELETE: `<strong>ADMIN</strong> deleted ${DATA?.content_type === 'FORUM' ? 'forum' : 'postingan'}.`,
                INVITE_FORUM: `<strong>${DATA.sender_name || 'Someone'}</strong> invited you to their forum.`,
                ADMIN_INVITE_FORUM: `<strong>ADMIN</strong> added you to a forum.`,
                WARNING: `<strong>ADMIN</strong> warned you about ${DATA.content_type === 'FORUM' ? 'forum' : 'postingan'}.`,
        ...
        async function pollForNotifications() {
            ...
            try {
                ...
                if (newNotifications.length > 0) {
                    ...
                    const currentCount = parseInt(unreadTabCount.textContent) || 0; // Ambil dari tab count
